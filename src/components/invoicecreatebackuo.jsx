...
import {
  ...
  const CreateInvoice = ({ setOpenCreateInvoice, type, invoice }) => {
    ...
    function itemsValidator() {
      const itemName = invoiceData.items.every((i) => validateItemName(i.name));
      const itemCount1 = invoiceData.items.every((i) => validateItemCount1(i.months));
      const itemCount = invoiceData.items.every((i) => validateItemCount(i.Delay));
      const itemPrice = invoiceData.items.every((i) => validateItemPrice(i.price));
      return itemName && itemCount1 && itemCount && itemPrice;
    }
